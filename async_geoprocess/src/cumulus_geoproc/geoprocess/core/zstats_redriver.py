...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--start", required=True, help="Date to start file checks in YYYYMMDD"
    )
    parser.add_argument(
        "--end", required=True, help="Date to end file checks in YYYYMMDD"
    )
    ...
    for dt in datetimes:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix=uuid4().__str__()) as td:
                ...
                outfile_shg = warp_to_vrt(
                    f"/vsicurl/{raster}",
                    os.path.join(td, out_filename),
                    extra_args=[
                        "-te",
                        "-356000",
                        "2494000",
            ...
            datetime_results = {
                "time_sec_download_warp": round(
                    _tend_download_warp - _tstart_download_warp
                ),
                "time_sec_stats": round(_tend_stats - _tstart_stats),
                "result": {
                    shape["properties"]["HU_10_NAME"]: format_properties(
                        shape["properties"]
                    )
                    for shape in zs
                },
            ...
            with open(
                os.path.join(
                    "/var/www/html/statistics_redriver",
                    f'statistics__{product}__{dt.strftime("%Y%m%d")}.json',
                ),
                "w",
            ) as day_outfile:
                day_outfile.write(json.dumps(datetime_results, separators=(",", ":")))
        except:
            # logging.error(
                f'Could not compute zonal_statistics; product: {product}; date: {dt.strftime("%Y%m%d")}'
            )

    # Write JSON Statistics File for Entire Run
    with open(
        os.path.join("/var/www/html/statistics_redriver", outfile), "w"
    ) as outfile:
