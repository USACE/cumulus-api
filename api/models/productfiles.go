...
func CreateProductfiles(db *pgxpool.Pool, ff []Productfile) (int, error) {
	...
	for _, f := range ff {
		if f.Version != nil {
			if _, err := tx.Exec(
				context.Background(),
				`INSERT INTO productfile (datetime, file, product_id, version) VALUES ($1, $2, $3, $4)
				 ON CONFLICT ON CONSTRAINT unique_product_version_datetime DO UPDATE SET update_date = CURRENT_TIMESTAMP`,
				 f.Datetime, f.File, f.ProductID, f.Version,
			); err != nil {
				return 0, err
			}
		} else {
			if _, err := tx.Exec(
				context.Background(),
				`INSERT INTO productfile (datetime, file, product_id) VALUES ($1, $2, $3)
				 ON CONFLICT ON CONSTRAINT unique_product_version_datetime DO UPDATE SET update_date = CURRENT_TIMESTAMP`,
				 f.Datetime, f.File, f.ProductID,
			); err != nil {
				return 0, err
			}
		}
