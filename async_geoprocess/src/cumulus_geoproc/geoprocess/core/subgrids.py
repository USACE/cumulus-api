...
def subgrids(productfile_object, mode="all"):
    ...
    if mode == "all":
        ...
        basin_envelopes = {
            b.id: {"envelope": b.mpoly.envelope, "office_id": b.office_id}
    ...
    buffered_basin_extents = {
        k: buffered_extent(v["envelope"].extent, 2, 2000)
        for k, v in basin_envelopes.items()
    }
    ...
    for uuid, extent in buffered_basin_extents.items():
        ...
        with open(NamedTemporaryFile(suffix=".tif").name, "w+b") as f:
            ...
            p.product = (
                productfile_object.product
            )  # Set FK of new product to parent product
            p.file = File(f)
