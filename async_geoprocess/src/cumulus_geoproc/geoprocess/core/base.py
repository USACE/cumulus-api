...
def write_array_to_raster(
    array, outfile, xsize, ysize, geotransform, projection, datatype, nodata_value
):

    dsout = gdal.GetDriverByName("GTiff").Create(
        outfile, xsize, ysize, 1, datatype, options=["COMPRESS=DEFLATE", "TILED=YES"]
    )
...
def create_overviews(
    infile, algorithm="average", levels=[2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048]
):
...
def interpolate(infile, outfile, max_distance, nodata):
    ...
    with tempfile.TemporaryDirectory(prefix=uuid4().__str__()) as td:
        ...
        _nodata = set_value_to_nodata(infile, os.path.join(td, f"_nodata.tif"), nodata)

        _filled = fill_nodata_values(
            _nodata, os.path.abspath(outfile), max_distance=max_distance
        )
